...
{
    let mut project_state = ProjectState{chord_sequence: ChordSequence::default()};
    let chord_pos = Tatum::try_from(0).unwrap();
    project_state.update_chord_sequence(chord_pos, Some(ChordDegree::II));
    assert_eq!(project_state.chord_sequence[chord_pos], Some(ChordDegree::II));
}

#[test]
fn update_chord_sequence_with_removing_chord()
{
    let mut project_state = ProjectState{chord_sequence: ChordSequence::new(Vec::from([Some(ChordDegree::II)])).unwrap()};
    let chord_pos = Tatum::try_from(0).unwrap();
